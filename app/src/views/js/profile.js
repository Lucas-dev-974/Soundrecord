...
export default {
  ...
  watch: {
    ...
    picture_file: function () {
      if (!this.overlay) this.update_picture();
      else {
        const reader = new FileReader();
        reader.addEventListener(
          "load",
          () => (this.banner_img = reader.result)
        );
  ...
  mounted() {
    ...
    if (this.$route.query && this.$route.query.userid) {
      ...
      // Else load user connected profile
  ...
  methods: {
    ...
    user_profile: function () {
      ...
      if (id) {
        api
          .get(
            "/api/profile?&userid=" + id + "&token=" + this.$store.state.token
          )
          .then(({ data }) => {
            // If creators field exist
            if (data.creator) {
              for (const creator_field in data.creator) {
                if (this[creator_field] != "undefined")
                  this[creator_field] = data.creator[creator_field];
              }
            }
            ...
            if (data.settings) {
              ...
              for (const setting in settings) {
                ["banner-img"];
                if (setting != null) {
                  if (this[setting.replace("-", "_")] != "undefined")
                    this[setting.replace("-", "_")] = settings[setting];
                }
              }
            }
    ...
    like_profile: function () {
      api
        .post("/api/like", { model: "creator", modelid: this.id })
        .then(({ data }) => {
      ...
      if (this.on_update == false) this.on_update = true;
        ...
        if (this.fields.length > 0) {
          ...
          this.fields.forEach((field) => {
            if (field == "public") params[field] = this.public_profile;
            else params[field] = this[field];
          });
    ...
    update_fields: function (field, field_setting = false) {
      if (!this.fields.includes(field) && !field_setting)
        this.fields.push(field);
      else if (field_setting && !this.settings_fields.includes(field))
        this.settings_fields.push(field);
    },
    ...
    update_banner: function (event) {
      if (typeof event == "object") this.overlay = false;
        ...
        if (this.picture_file) {
          ...
          api
            .post("/api/profile-setting/banner-upload", form_data)
            .then(({ data }) => {
              console.log(data);
            })
            .catch((error) => {
        ...
        if (
          this.$store.state.profile_settings["banner-color"] &&
          this.$store.state.profile_settings["banner-color"] !=
            this.banner_color
        ) {
          this.$store.commit("pusProfileSettings", {
            "banner-color": this.banner_color,
          });
          api
            .patch("/api/profile-setting", {
              setting_name: "banner-color",
              value: this.banner_color,
            })
            .then(({ data }) => {
              console.log(data);
            })
