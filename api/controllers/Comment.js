  ...
  getAudioComments: async function (req, res) {
    ...
    for (const comment of comments) {
      ...
      if (comment.dataValues.responseof != null) {
        ...
        _comments[comment.dataValues.id] = comment.dataValues;
      }
    }
  ...
  post: async function (req, res) {
    ...
    if (!audio)
      return res
        .status(404)
        .json(
          "Il n'est pas possible de placer un commentaire sous un audio qui n'existe pas."
        );
  ...
  edit: async function (req, res) {
    ...
    if (comment.userid != req.user.id) {
      return res
        .status(401)
        .json(
          "Vous ne pouvez pas modifier un comment qui n'est pas le votre !"
        );
    }
