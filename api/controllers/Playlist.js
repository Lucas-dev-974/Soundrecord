...
module.exports = {
    ...
    one: async function(req, res){
        ...
        if(playlistInformations.UserId != req.user.id) return res.status(200).json("Vous ne pouvez pas accéder à cet ressoure.")
    ...
    remove: async function(req, res){
        ...
        const playlist = await models.Playlist.findOne({where: {id: validated.id}})
        if(playlist.UserId != req.user.id) return res.status(401).json("vous n'avez pas l'authorisation !")
    ...
    addAudio: async function(req, res){
        ...
        const audio = await models.Audio.findOne({where: {id: validated.audioid}})
        if(!audio) return res.status(404).json("L'audio n'existe plus.")
        if(!audio.public && audio.UserId != req.user.id) return res.status(401).json("Désoler vous ne pouvez pas ajouter cet audio")
        
        const playlist = await models.Playlist.findOne({where: {id: validated.playlistid}})
        if(!playlist || playlist.UserId != req.user.id) return res.status(404).json("Vous ne pouvez pas accédé à cet playlist.")
    ...
    removeAudio: async function(req, res){
        ...
        const playlistAudio = await models.PlaylistAudio.findOne({where: validated})
        if(!playlistAudio) return res.status(404).json("L'audio n'existe pas dans la playlist.")
