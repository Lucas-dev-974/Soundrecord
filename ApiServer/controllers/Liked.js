...
module.exports = {
    get: async function(req, res){
        ...
        if(validated.errorSize > 0) return res.status(403).json({error: 'Désoler une erreur est survenue'})
        
        if(!_models.includes(validated.validated.model)) return res.status(403).json({error: 'Désoler l\'entité demandé ne peux être transmit'})

        const likes = await models.Liked.findAndCountAll({ where: { 
